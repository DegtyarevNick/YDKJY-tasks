...
function scheduleMeeting(startTime, durationMinutes) {
  ...
  if (
    ...
    if (time[1] + durationMinutes >= 60) {
      ...
      time[1] = time[1] + durationMinutes - 60 * Math.floor((time[1] + durationMinutes) / 60) == 0
          ? "00"
          : time[1] + durationMinutes - 60 * Math.floor((time[1] + durationMinutes) / 60); // converting minutes for comparison(0 => 00)
    } else {
